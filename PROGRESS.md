...
- **Son tamamlanan gün:** Gün 4 — İP-16: ortak TDA (3. derece) arayüz bileşenleri (MK-53) `biocraft-ui` altında egui ile + standart hata şeması `biocraft-types` içine (2026-06-20)
- **Sıradaki gün:** Gün 5 — İP-04: winit + wgpu + egui pencere (render host); açılışta <500 ms "merhaba" penceresi + örnek TDA galerisini canlı gösterme
- **Derleme durumu:** ✅ `cargo build --workspace` (13 crate) + **51 test** geçiyor (types 24 + ui 27); fmt/clippy(-D warnings)/machete/topology/**cargo-deny** temiz. egui 0.29 eklendi; deny.toml'a font lisansları (OFL-1.1 + UFL-1.0 muafiyeti) eklendi. GitHub Actions: 3 job (ubuntu+windows+deny) — Gün 3'te canlı doğrulandı; Gün 4 değişiklikleri henüz CI'da koşmadı (push sonrası)
