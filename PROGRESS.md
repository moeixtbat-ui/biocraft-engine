...
- **Son tamamlanan gün:** Gün 7 — İP-08 (Bellek Orkestratörü + Bütçe + Out-of-core): `biocraft-mem` altında **Global Memory Orchestrator** (rezervasyonla tahsis; bütçe aşımında **OOM-çökmesiz reddetme** + standart hata; bellek baskısında **LRU önbellek boşaltma** — MK-21/MK-22), **dosya-öncesi bütçe kontrolü** (boyut + tahmini RAM → akış/cloud-burst/iptal teklifi), **öncelik modları** (Arayüz/Denge/Maksimum + Zero-Impact hesap-kısma kancası), **out-of-core akış** (stream + mmap; büyük veri pencere pencere, "load all" yok — MK-09). UI tarafında bütçe diyaloğu bileşeni + galeride canlı bellek/öncelik/akış demosu. Donanım termal koruma Gün 8'e bırakıldı. (2026-06-21)
- **Sıradaki gün:** Gün 8 — İP-08 Donanım Koruma (Zero-Impact'in donanım tarafı): termal eşik tablosu (GPU/CPU/NVMe kademeli yük azaltma + kritikte durdurma + checkpoint), bağımsız watchdog izleme thread'i, disk doluluk koruması (%10 uyarı / %2 salt-okunur + güvenlik marjı), durum çubuğuna donanım göstergesi (CPU/GPU/RAM/sıcaklık)
- **Derleme durumu:** ✅ `cargo build --workspace` (13 crate) + **140 test** geçiyor (types 24 + ui 36 + render 50 + **mem 30**); fmt/clippy(-D warnings)/machete/topology/**cargo-deny (exit 0)** temiz. Yeni bağımlılık: **memmap2** (out-of-core mmap yolu; MIT/Apache-2.0, deny ok). `biocraft-ui` artık `biocraft-mem`'e bağlı (L4→L2; topoloji temiz). `biocraft-mem` projede `Result<_, ErrorReport>` döndüren ilk crate; `ErrorReport` büyük olduğundan gerekçeli crate-seviyesi `#![allow(clippy::result_large_err)]` (bkz. lib.rs notu). **Çalışma-zamanı:** galeri'nin yeni İP-08 bölümü (bellek çubuğu + rezerve/önbellek/baskı butonları + bütçe diyaloğu + akış + öncelik) tüm tema/dil kombinasyonlarında headless egui testiyle çökmeden çiziliyor (canlı pencere görsel doğrulaması kullanıcıda). cargo-deny: 14 zararsız "duplicate version" uyarısı (exit 0). GitHub Actions: 3 job — Gün 6 push'u CI'da doğrulanacak; Gün 7 push'u sonrası izlenecek
