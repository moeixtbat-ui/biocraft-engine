...
- **Son tamamlanan gün:** Gün 6 — İP-04 tamamlama (render + tasarım): `assets/tokens.json` tabanlı tema sistemi (Koyu/Açık/Yüksek-kontrast + **özel tema E2**), tipografi (Inter/JetBrains Mono/Space Grotesk + DPI), 2B plot (coverage/çizgi/scatter + culling/LOD), 3B sahne (native wgpu top-çubuk, off-screen doku → egui), görünür-alan culling + LOD API'si. **Kodda hiç sabit renk yok** (MK-52); tüm renkler token'dan (MK-52) (2026-06-20)
- **Sıradaki gün:** Gün 7 — İP-08: Bellek Orkestratörü + Performans + Donanım Koruma (Zero-Impact); render/iş katmanları bellek bütçesinden rezervasyon ister (MK-21/MK-22). Durum çubuğuna donanım göstergesi (CPU/GPU/RAM/sıcaklık) eklenebilir
- **Derleme durumu:** ✅ `cargo build --workspace` (13 crate) + **108 test** geçiyor (types 24 + ui 34 + render 50); fmt/clippy(-D warnings)/machete/topology/**cargo-deny (exit 0)** temiz. Yeni bağımlılıklar: serde + serde_json (token JSON) + bytemuck (3B POD) — hepsi lisans politikasına uyuyor (deny ok). **MK-52 güvencesi:** kaynak ağacında hiç sabit renk yok (`from_rgb`/`#RRGGBB`/named-color sıfır; grep ile doğrulandı); tek renk kaynağı `assets/tokens.json`. Çalışma-zamanı doğrulandı: `--cpu` WARP ile pencere açıldı, **WGSL shader derlendi + 3B pipeline kuruldu + off-screen doku egui'ye bağlandı, çökme yok** (gerçek GPU görsel doğrulaması kullanıcıda). cargo-deny: 14 "duplicate version" uyarısı (zararsız, exit 0). GitHub Actions: 3 job (ubuntu+windows+deny) — Gün 5 push'u (d196c25) yeşil doğrulanmıştı; Gün 6 push'u CI'da doğrulanacak
